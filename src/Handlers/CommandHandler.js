...
class CommandHandler
{
	...
	handle(message, prefix, NHLClient)
	{
		...
		if(MiscCommands[command]) {
			...
			for(var key in commandList) {
				...
				if (keys.includes(command)) {
					this.respondCommand(message, key, command, NHLClient);
				}
			}
		}
	}

	respondCommand(message, key, command, NHLClient)
	{
		let commandList = CommandManager.getCommandList();
		if(this.commandsMap.has(key)) {
			...
			commandObject.handle(message, NHLClient);
		}
	}
